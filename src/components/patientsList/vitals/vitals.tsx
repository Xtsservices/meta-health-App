...
export default function VitalsTabScreen() {
  ...
  const fetchVitals = useCallback(async () => {
    ...
    try {
      ...
      // Trigger AI Analysis for the latest vitals
      if (normalized.length > 0) {
        const latest = normalized[0];
        if (latest.pulse && latest.temperature && latest.oxygen) {
          getAIAnalysis(latest, token);
        }
      }
    } finally {
      setLoading(false);
  ...
  const getAIAnalysis = async (vitalData: VitalRow, token: string | null) => {
    setIsAnalyzing(true);
    try {
      const bpH = vitalData.bp?.split("/")[0] || "";
      const bpL = vitalData.bp?.split("/")[1] || "";
    ...
    return (
      <View style={styles.aiCard}>
        <View style={styles.aiHeader}>
          ...
          <Text style={styles.aiTitle}>AI Vitals Insights</Text>
          {isAnalyzing && <ActivityIndicator size="small" color="#0056b3" style={{ marginLeft: 10 }} />}
        </View>
        <Text style={styles.aiText}>
          {aiAnalysis || "Analyzing current vitals..."}
        </Text>
    ...
    <View style={[styles.safe, { backgroundColor: COLORS.bg }]}>
      {loading && rows.length === 0 ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={<AIAnalysisHeader />}
