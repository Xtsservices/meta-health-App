...
const MedicationMissedAlertsTab: React.FC<{
  navigation: any;
  refreshing: boolean;
  onRefresh: () => void;
  ...
  const fetchMedicationAlerts = useCallback(async () => {
    try {
      ...
      if (response?.status === "success") {
        setAlerts(response?.data?.data?.medicationMissedAlerts || []);
      } else {
        dispatch(showError('Failed to fetch missed medication alerts'));
      }
    } catch (error: any) {
      dispatch(
        showError(
          ...
          error?.message ||
          "Failed to fetch missed medication alerts"
        )
      );
    } finally {
      setLoading(false);
  ...
  const getTimeElapsed = (dosageTime?: string) => {
    if (!dosageTime) return "—";
    
    try {
      const dosageDate = new Date(dosageTime);
      if (isNaN(dosageDate.getTime())) return "—";
      
      const now = new Date();
      const diffMs = now.getTime() - dosageDate.getTime();
      const diffMinutes = Math.floor(diffMs / (1000 * 60));
      
      if (diffMinutes < 0) return "In future";
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>
          Loading Medication Missed Alerts...
        </Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.emptyState}>
        ...
        <Text style={styles.emptySubtitle}>
          All medications are administered on time
        </Text>
      </View>
    );
  }
...
export default MedicationMissedAlertsTab;